...
def test_authoritative_period_replaces_model_temporal_filters() -> None:
    ...
    assert all([f.field for f in q.query.filters] == ["overtime.status"] for q in resolved.queries)
...
def test_period_list_replaces_wrong_payroll_field_with_subject_temporal_field() -> None:
    ...
    resolved = _apply_temporal_intent(
        plan,
        TemporalIntent(kind="period_list", months=[1, 3, 6], year=2026),
        context(date(2026, 8, 29)),
        catalog,
    )
    assert len(resolved.queries) == 3
    assert [item.query.time_scope.field for item in resolved.queries] == ["overtime.work_date"] * 3
...
def test_genuine_payroll_period_scope_is_preserved_for_payroll_subject() -> None:
    ...
    resolved = _apply_temporal_intent(
        plan,
        TemporalIntent(kind="explicit_month_year", month=1, year=2026),
        context(date(2031, 4, 17)),
        catalog,
    )
