...
| 2026-09-12 | Ajuste de auditoria/UI para Human Review: la bandeja deja de mostrar acciones sobre items ya decididos, los comentarios del revisor se exponen en la lectura y detalle del analisis, y la UI diferencia estados con indicadores verde/naranja/rojo. Se mantiene `needs_information` como estado terminal actual y queda recomendada su evolucion a ciclo de aclaracion/resubmision. | `apps/peopleops-api/src/peopleops_api/main.py`, `apps/peopleops-api/src/peopleops_api/schemas.py`, `apps/peopleops-api/tests/test_persistence.py`, `apps/peopleops-web/app/human-review/page.tsx`, `apps/peopleops-web/app/page.tsx`, `apps/peopleops-web/app/globals.css`, `docs/SEMANTIC-ANALYSIS-CAPABILITY-PLAN.md`. | `poetry run pytest tests/test_persistence.py::test_analysis_api_exposes_human_review_comment_in_summary_and_details tests/test_human_review.py::test_reject_and_needs_information_resume_same_request_id -q` => 3 passed; `poetry run ruff check src/peopleops_api/main.py src/peopleops_api/schemas.py tests/test_persistence.py` => passed; `npm run lint` y `npm run build` en `apps/peopleops-web` => passed. | `IN_PROGRESS` |

## Flujo aprobado para `needs_information`

`needs_information` deja de ser una conclusión terminal. La decisión pausa el
análisis original en `waiting_for_user_information`, conserva el comentario
del revisor y habilita una respuesta del usuario. Esa respuesta crea una nueva
`AnalysisInteraction` con nuevo `request_id`, misma `conversation_id` y vínculo
al análisis original. La continuación incorpora la información como contexto
adicional no confiable y ejecuta nuevamente el workflow completo, incluyendo
autorización, MCP, políticas, evidencia y Human Review. El análisis original y
la continuación permanecen visibles y auditables por separado.

La información entregada por el usuario nunca concede permisos ni reemplaza
la decisión humana; si la continuación vuelve a requerir revisión, se crea un
nuevo item de Human Review para esa nueva interacción.

| 2026-09-12 | Flujo documentado e implementado para `needs_information`: pausa durable en `waiting_for_user_information`, comentario del revisor visible, formulario de respuesta del usuario y continuación con nuevo `request_id` enlazado al análisis original. La continuación repite el workflow completo y puede generar una nueva revisión. | `docs/slice-10-human-in-the-loop.md`, `docs/SEMANTIC-ANALYSIS-CAPABILITY-PLAN.md`, runtime API/UI y migración 0005 | `pytest` completo, Ruff, lint y build frontend pasaron. Alembic requiere una base accesible para validación de migración. | `IN_PROGRESS` |
| 2026-09-12 | Flujo implementado: AnalysisInteraction persiste el vínculo de continuación y el contexto adicional; needs_information queda en waiting_for_user_information; la API crea y ejecuta una continuación con nuevo request_id; la UI permite responder desde Análisis y conserva ambos casos en historial. | apps/peopleops-api/src/peopleops_api/models.py, apps/peopleops-api/src/peopleops_api/repositories.py, apps/peopleops-api/src/peopleops_api/analysis_workflow.py, apps/peopleops-api/src/peopleops_api/main.py, apps/peopleops-api/alembic/versions/0005_information_continuation.py, apps/peopleops-web/app/page.tsx, docs/slice-10-human-in-the-loop.md | pytest tests/test_human_review.py tests/test_persistence.py -q => 14 passed; Ruff, npm lint y npm build pasaron. Alembic no pudo conectarse porque el host peopleops-db no es resoluble en el entorno actual. | IN_PROGRESS |
